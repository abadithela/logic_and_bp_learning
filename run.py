...
def main(env_name, headless):
    ...
    final_policy_path = os.path.join(model_dir, final_policy_name)
    ...
    if env_name == 'lake':
        ...
        fitted_off_policy_evaluation_algorithm = LakeFittedQEvaluation(initial_states, 
                                                           ...
                                                           position_of_holes=position_of_holes,
                                                           num_frame_stack=num_frame_stack)
        exact_policy_algorithm = ExactPolicyEvaluator(action_space_map, gamma, env=env, frame_skip=frame_skip, num_frame_stack=num_frame_stack, pic_size = pic_size, constraints_cared_about=constraints_cared_about)
    elif env_name == 'car':
...
=starting_lambda)
    ...
    if env_name in ['lake']:
        ...
        problem.dataset['g'] = problem.dataset['g'][:,constraints_cared_about]
